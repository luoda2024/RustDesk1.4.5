...
const luoda_CLIPBOARD_OWNER_FORMAT: &'static str = "dyn.com.luoda.owner";
...
const SUPPORTED_FORMATS: &[ClipboardFormat] = &[
    ...
    ClipboardFormat::Special(luoda_CLIPBOARD_OWNER_FORMAT),
];
...
pub fn is_file_url_set_by_luoda(url: &Vec<String>) -> bool {
    if url.len() != 1 {
        return false;
    }
    url.iter()
        .next()
        .map(|s| {
            for prefix in &["file:///tmp/.luoda_", "//tmp/.luoda_"] {
...
fn do_update_clipboard_(mut to_update_data: Vec<ClipboardData>, side: ClipboardSide) {
    ...
    if let Some(ctx) = ctx.as_mut() {
        to_update_data.push(ClipboardData::Special((
            luoda_CLIPBOARD_OWNER_FORMAT.to_owned(),
            side.get_owner_data(),
        )));
...
impl ClipboardContext {
    ...
    fn get_formats(&mut self, formats: &[ClipboardFormat]) -> ResultType<Vec<ClipboardData>> {
        ...
        // See https://github.com/luoda-org/arboard/blob/747ab2d9b40a5c9c5102051cf3b0bb38b4845e60/src/platform/windows.rs#L34
        //
        // This is a common case on Windows, so we retry here.
        // Related issues:
        // https://github.com/luoda/luoda/issues/9263
        // https://github.com/luoda/luoda/issues/9222#issuecomment-2329233175
    ...
    fn get_formats_filter(
        ...
        if !force {
            for c in data.iter() {
                if let ClipboardData::Special((s, d)) = c {
                    if s == luoda_CLIPBOARD_OWNER_FORMAT && side.is_owner(d) {
                        return Ok(vec![]);
                    }
                }
            }
        ...
        Ok(data
            .into_iter()
            .filter(|c| match c {
                ClipboardData::Special((s, _)) => s != luoda_CLIPBOARD_OWNER_FORMAT,
    ...
    #[cfg(all(feature = "unix-file-copy-paste", target_os = "macos"))]
    fn get_file_urls_set_by_luoda(
        data: Vec<ClipboardData>,
        _side: ClipboardSide,
    ) -> Vec<String> {
        for item in data.into_iter() {
            if let ClipboardData::FileUrl(urls) = item {
                if is_file_url_set_by_luoda(&urls) {
                    return urls;
                }
            }
        }
    ...
    fn get_file_urls_set_by_luoda(data: Vec<ClipboardData>, side: ClipboardSide) -> Vec<String> {
        let exclude_path =
        ...
        if let Ok(data) = self.get_formats(&[ClipboardFormat::FileUrl]) {
            ...
            if !urls.is_empty() {
                ...
                let clear_holder_text = if is_kde_x11 {
                    "luoda placeholder to clear the file clipbard"
                } else {
                    ""
                }
                .to_string();
...
// https://github.com/luoda-org/clipboard-master/blob/4fb62e5b62fb6350d82b571ec7ba94b3cd466695/src/master/x11.rs#L226
#[cfg(not(target_os = "android"))]
