...
const MSG_TO_luoda_TARGET: &str = "luoda";
...
const MSG_TO_luoda_SIGNATURE_VERIFICATION: &str = "signature_verification";
...
pub(super) extern "C" fn cb_msg(
    ...
    match &target as _ {
        ...
        MSG_TO_luoda_TARGET => handle_msg_to_luoda(id, content, len),
        _ => PluginReturn::new(
...
fn handle_msg_to_luoda(id: String, content: *const c_void, len: usize) -> PluginReturn {
    let s = early_return_value!(
        std::str::from_utf8(unsafe { std::slice::from_raw_parts(content as _, len) }),
        ERR_CALLBACK_INVALID_MSG,
        "parse msg string"
    );
    let msg_to_luoda = early_return_value!(
        serde_json::from_str::<MsgToluoda>(s),
        ERR_CALLBACK_INVALID_MSG,
        "parse msg '{}'",
        s
    );
    match &msg_to_luoda.r#type as &str {
        MSG_TO_luoda_SIGNATURE_VERIFICATION => request_plugin_sign(id, msg_to_luoda),
        t => PluginReturn::new(
            errno::ERR_CALLBACK_TARGET_TYPE,
            &format!(
                "Unknown target type '{}' for target {}",
                t, MSG_TO_luoda_TARGET
            ),
        ),
    }
}

fn request_plugin_sign(id: String, msg_to_luoda: MsgToluoda) -> PluginReturn {
    let signature_data = early_return_value!(
        std::str::from_utf8(&msg_to_luoda.data),
        ERR_CALLBACK_INVALID_MSG,
